...
const InvoiceTable: React.FC<InvoiceTableProps> = ({ bill, isSelectable = true, isLoadingBill, onSelectItem }) => {
  ...
  const [selectedLineItems, setSelectedLineItems] = useState(lineItems ?? []);
  const [searchTerm, setSearchTerm] = useState('');
  const debouncedSearchTerm = useDebounce(searchTerm);

  // Notify parent component when all items are selected initially or when lineItems change
  useEffect(() => {
    if (lineItems && lineItems.length > 0) {
      setSelectedLineItems(lineItems);
      if (onSelectItem) {
        onSelectItem(lineItems);
      }
    }
