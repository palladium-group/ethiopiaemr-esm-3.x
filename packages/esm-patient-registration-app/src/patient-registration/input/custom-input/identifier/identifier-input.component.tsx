...
const IdentifierInput: React.FC<IdentifierInputProps> = ({ patientIdentifier, fieldName }) => {
  ...
  const { defaultPatientIdentifierTypes, healthIdIdentifierTypeUuid } = useConfig<RegistrationConfig>();
  const { identifierTypes } = useResourcesContext();
  const { isOffline, values, setFieldValue, inEditMode, isEmpiDemographicsLocked } = usePatientRegistrationContext();
  ...
  const hideOptionalLabel = !!defaultPatientIdentifierTypesMap[patientIdentifier.identifierTypeUuid];
  const isHealthIdIdentifier =
    !!healthIdIdentifierTypeUuid && patientIdentifier.identifierTypeUuid === healthIdIdentifierTypeUuid;
  const lockHealthIdInEditMode = !!inEditMode && isEmpiDemographicsLocked && isHealthIdIdentifier;
  ...
  }, [initialValue, lockHealthIdInEditMode, setHideInputField]);
  ...
  const showEditButton =
    !lockHealthIdInEditMode && !required && hideInputField && (!!initialValue || manualEntryEnabled);
  const showResetButton =
    (!lockHealthIdInEditMode && !!initialValue && initialValue !== identifierValue) ||
    (!hideInputField && manualEntryEnabled);
  return (
      ...
      {!hideInputField ? (
        <Field name={name} validate={validateInput}>
          {({ field, form: { touched, errors } }) => (
            <Input
              ...
              name={name}
              disabled={disabled || lockHealthIdInEditMode}
        ...
        <div className={styles.textID}>
          <p data-testid="identifier-label" className={styles.label}>
            {required || hideOptionalLabel ? identifierName : `${t('optionalIdentifierLabel', { identifierName })}`}
          </p>
